# quant_pilot_1 (public release)

This is the public-release version of the Challenge Fund Quantitative Study Pilot 1 grading pipeline, the canonical de-identified dataset (`data/cleaned/quant-1-pilot-no-ids.xlsx`), the descriptive-analysis code under `src/analyses/descriptives/`, and the descriptive-analysis outputs under `data/analyses/descriptives/`. Several upstream artefacts have been withheld for PII reasons; the next section enumerates them.
...
- `data/raw/quant-1-pilot.csv` — raw GuidedTrack export (held `PROLIFIC_PID` and self-reported demographics).
- `data/cleaned/quant-1-pilot.xlsx` — with-IDs cleaned dataset (held the same identity and demographic columns).
- `data/cleaned/warnings.txt` — runtime warnings emitted by stage 1 (individual warning lines quote `PROLIFIC_PID` values).

The two stage-1+2 scripts that consumed those inputs (`process_quant_pilot.py`, `make_no_ids_xlsx.py`) are preserved under `src/pipeline_record/` as a record of how the de-identified dataset was produced. They are not runnable from this distribution and will exit immediately with an explanatory `FileNotFoundError` if invoked. `data/cleaned/quant-1-pilot-no-ids.xlsx` is the canonical entry point for downstream analyses; anything that previously read `quant-1-pilot.xlsx` should read the `-no-ids` version instead.
...
│   │   └── make_no_ids_xlsx.py
│   └── analyses/
│       └── descriptives/               # 15 descriptive-analysis scripts (stage 3)
├── docs/
...
This runs the 15 stage-3 scripts under `src/analyses/descriptives/` against the shipped `quant-1-pilot-no-ids.xlsx` and writes their outputs to `data/analyses/descriptives/`.

To run a single descriptives script:

```bash
python src/analyses/descriptives/credence_change.py \
    --input data/cleaned/quant-1-pilot-no-ids.xlsx \
    --mapping lookups/debate-to-name-and-question-mapping.csv \
    --out-dir data/analyses/descriptives/
```

`make clean` removes only the regenerable analysis outputs under `data/analyses/descriptives/`. The shipped `quant-1-pilot-no-ids.xlsx` is not regenerable from this distribution and is intentionally preserved across `make clean`.
...
Every analysis script under `src/analyses/` MUST read `data/cleaned/quant-1-pilot-no-ids.xlsx`, never `quant-1-pilot.xlsx`. The Makefile encodes this: the `descriptives` target depends on `$(NOIDS_XLSX)`, not on the (withheld) with-IDs file. Any new analysis script added under `src/analyses/` should follow this rule.

## What each folder is for

- **`data/raw/`** — would hold the immutable inputs; contents withheld in this release.
- **`data/cleaned/`** — `quant-1-pilot-no-ids.xlsx` is canonical; upstream variants withheld.
