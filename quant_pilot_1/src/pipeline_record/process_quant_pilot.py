...
public distribution: the raw input (`data/raw/quant-1-pilot.csv`) was withheld
because it retains `PROLIFIC_PID` and other identity columns, and the
with-IDs output it would write (`data/cleaned/quant-1-pilot.xlsx`) was
withheld for the same reason. The de-identified equivalent is shipped at
`data/cleaned/quant-1-pilot-no-ids.xlsx`. See `src/pipeline_record/README.md`.

Drops 48 duplicate question-text columns, decodes hex-encoded answers,
extracts timing and credence data, merges assignment/condition metadata from
the lookup tables, filters rows that fail validity checks, and writes a
formatted XLSX with clickable URL columns.
"""
...
# ── Columns to keep (drop 48 question-text duplicates) ──────────────────────
...
def process(input_csv: Path, lookups_dir: Path, output_xlsx: Path, warnings_path: Path):
    ...
    name_map = pd.read_csv(
        lookups_dir / "debate-to-name-mapping.csv", encoding="utf-8",
    )
    id_to_name = {
