...
with the Student-t secondary fit in place of the Normal-residual secondary.

Same structure and layout as plot_outcome_hdis.py: two panels (primary on
the left, secondary on the right), one row per transcript, horizontal line
from HDI_lo to HDI_hi with a dot at the mean, coloured by domain. The only
difference is the right panel uses output/secondary-t-fit.nc.

The Student-t fit has identical fixed/random-effect structure to the
Normal-residual secondary fit (only the residual likelihood differs), so
the per-transcript mu_v formula is unchanged. Comparing this PNG to
outcome-hdi-forest.png shows how the heavy-tail likelihood affects the
per-transcript HDIs.

Output: output/outcome-hdi-forest-t.png
"""
...
def plot_forest(prim, sec_t, out_path):
...
mu_sec_t = compute_per_transcript_mu(
    az.from_netcdf(OUT_DIR / "secondary-t-fit.nc"), lookup
)

plot_forest(
    summarize(mu_prim, lookup),
    summarize(mu_sec_t, lookup),
    OUT_DIR / "outcome-hdi-forest-t.png",
)
