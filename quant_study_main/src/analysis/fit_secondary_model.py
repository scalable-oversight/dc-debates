"""Fit the secondary Bayesian hierarchical model from CF_analysis_plan.docx.

Identical specification to the primary model, except the outcome is
y_secondary = (logit credence in honest's answer after part 4)
            - (logit credence in honest's answer after part 1).

Model:
    y_secondary_i ~ Normal(mu_i, sigma_residual)
    mu_i = alpha
         + beta_order * order_i
...
Output: <out_dir>/secondary-fit.nc  (ArviZ InferenceData via NetCDF).
"""
