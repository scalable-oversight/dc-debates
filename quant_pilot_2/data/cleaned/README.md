...
This directory's canonical content is `quant-1-pilot-2-no-ids.xlsx`, the de-identified cleaned dataset that is the input to stage 3 of the pipeline and any downstream analysis.

Two upstream artefacts were withheld from the public release:

- `quant-1-pilot-2.xlsx` — the with-IDs cleaned XLSX produced by `process_quant_pilot.py`. Retained `PROLIFIC_PID` and self-reported demographic columns.
